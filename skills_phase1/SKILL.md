...
2. **所有 SN 做完才算完。** 不允许只做高优先级 SN 就切换到 Stage 2。progress.md 中每个 SN 的 L0-L4 都必须填满，这是 Stage 1 唯一的退出条件（详见「Stage 1 完成门禁」）。
3. **GPU 是参照不是答案。** 对齐优化意图，用 NPU 方法实现。
4. **Forward + backward + 真实 shape 才算有效 benchmark。**
5. **单机 benchmark ≠ 多卡增益。** 必须 ablation 验证。
6. **改动成本越低越优先尝试。**
7. **不因单次失败放弃。** 追根因，换方式再试。
8. **区分"移植遗留"和"数学语义"。** 用户要求"不改数学语义"时，移除 GPU 原始代码中不存在的移植遗留代码（如多余的 `.float()`）是**恢复**原始语义，不是修改语义。详见 Step 3a「精度对齐」。
...
**这是 Stage 1 结束、切换到 Stage 2 的唯一合法出口。** Agent 不允许在未通过此门禁的情况下以任何理由进入 Stage 2。

**门禁检查流程（必须逐步执行并输出结果）**：

1. **读取 progress.md**：`cat benchmarks/supernodes/progress.md`，将完整表格输出给用户
2. **逐 SN 检查**：对表中每个 SN，检查 L0-L4 的每个格子：
   - ✅ / ✅cond / ❌+原因 / skip:原因 → 已完成
   - `-`（未开始）→ **阻止通过**，必须先完成
   - 🔄（进行中）→ **阻止通过**，必须先完成
   - ❌ 但无原因 → **阻止通过**，必须补充原因
3. **输出完成度统计**：
   ...
   Stage 1 完成门禁检查:
   - 总 SN 数: X
   - 完成 SN 数: X（L0-L4 所有格子已填满）
   - 未完成 SN 数: X
     - SN-xxx: L2 未开始, L3 未开始
     - SN-yyy: L1 进行中
   - 完成率: XX%
   - 门禁结果: 通过 / 未通过（原因: ...）
   ```
4. **只有完成率 = 100% 且门禁结果 = 通过时**，才允许声明 "Stage 1 结束" 并切换到 Stage 2

**常见违规模式（agent 必须自检避免）**：
- ❌ 只做了高优先级 SN 就想切换 → 所有 SN 都要做完
- ❌ 某个 SN 的 L2/L3 跳过但没写 skip 原因 → 补上原因
- ❌ progress.md 没有更新到最新状态 → 先更新再检查
- ❌ 声称"GPU IR 不够用"来跳过剩余 SN → 有 IR 就继续分析，IR 不覆盖的部分在格子里标注"无 IR 参照，已用 NPU-only 方法测试"
- ❌ 声称"对话太长需要切换" → 记录 progress，下次继续，不切换
