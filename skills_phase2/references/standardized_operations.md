...
7. **训练场景短跑策略**：训练场景的 profiling 和迭代收益确认**必须基于短跑脚本**（等比压缩 total steps，~60-120 秒覆盖所有 regime），禁止在完整训练流程上采集。短跑脚本构造一次后在整个优化迭代中复用。详见 [profiling_collection.md](../01_preparation/references/profiling_collection.md) §训练短跑策略。
