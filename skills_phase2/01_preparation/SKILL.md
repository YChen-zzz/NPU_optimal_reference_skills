...
### 训练短跑脚本（训练场景强制，Phase 1 产出）

训练场景的 profiling 和迭代验证**必须基于短跑脚本**，不在完整训练上采集。短跑脚本是 Phase 1 的关键产出之一，构造一次后在整个优化迭代中复用。

**短跑脚本用途**（贯穿全流程）：
- Phase 1：采集基线 wall-clock（step_avg）+ L0
- Phase 2：采集 L1 profiling 用于瓶颈分析
- Phase 4：采集 wall-clock + L0 确认优化收益
- 最终确认：仍使用 full training 验证绝对性能（短跑不替代）

**构造规则**：等比压缩 total steps（1/10~1/20），按相同比例映射所有 step-dependent schedule，覆盖所有 regime，运行时间控制在 ~60-120 秒。详见 [profiling_collection.md](references/profiling_collection.md) §训练短跑策略。
...
wall-clock（无 profiler 的真实时间）、L0（设备执行时间）、L1（瓶颈分析数据）三种测量的采集方式不同，但**必须覆盖完全相同的代码范围**（模型全部功能代码）。训练场景下三种测量均基于同一个短跑脚本，仅开关 profiler 不同。详见 [profiling_collection.md](references/profiling_collection.md) §三种性能测量及其覆盖范围。
...
→ 1. 确定采集场景（训练 / 推理）
→ 2. 训练场景：构造短跑脚本（推理场景跳过此步）
→ 3. 选择采集级别（L0 /  L1 ）
→ 4. 植入 Profiling 代码（按框架选择模板）
→ 5. 设置环境变量（TASK_QUEUE_ENABLE, CPU_AFFINITY_CONF）
→ 6. 执行采集（训练：运行短跑脚本；推理：运行推理函数），产出 trace 文件
→ 7. 进入 Phase 2 分析
```
