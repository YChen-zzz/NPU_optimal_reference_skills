...
**Phase 2 Profiling 分析**：采集性能数据，定位瓶颈类型（计算密集 / 访存密集 / Host-Bound），输出优先级排序的优化清单。

**Phase 3 优化实施**：根据 profiling 一次性提出多条优化点，逐条实施并验证，批次完成后全量验证 + git commit，然后重新 profiling 确认瓶颈转移。图编译优先尝试，失败后走 eager 路线。
