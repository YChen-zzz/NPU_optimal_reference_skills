...
1. **可归因对比**：每个候选或兼容 bundle 都必须保留可回滚边界，并用同口径 wall-clock 比较。仅当需要证明机制、收益异常或触发 stall 时采 L0/L1；不为每个 Action 强制重采 Profiling。
2. **采纳条件**：新配置在目标指标上改善且不越过 accuracy、内存、编译时间和稳定性约束时才采纳。出现取舍时按任务合同自动判定，并记录理由。
