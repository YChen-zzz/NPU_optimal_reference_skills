...
3. **候选清单**（见 [profiling_to_action.md](../02_bottleneck_analysis/references/profiling_to_action.md) §收益排序）：
   - 每条候选含：问题 + 位置 + 影响范围 + 量化上限（引用报告输出数值）
   - 每条候选须标注"消除的开销类别"（对应优先级覆盖表的浪费类别）和"理论收益上限"（对应该类浪费的量化上限）
   - 候选按理论收益上限降序排列
